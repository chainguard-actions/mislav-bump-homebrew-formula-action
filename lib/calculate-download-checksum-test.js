...
import { URL } from 'url';
import { parseArchiveUrl, parseReleaseDownloadUrl, resolveRedirect, } from './calculate-download-checksum.js';
...
test('calculate-download-checksum resolveRedirect follows HTTP 303', async (t) => {
    ...
    const url = await resolveRedirect(apiClient, new URL(`http://localhost:${address.port}/tarball`), false);
    t.is(url.href, 'https://codeload.example.com/tar.gz');
    const relativeUrl = await resolveRedirect(apiClient, new URL(`http://localhost:${address.port}/tarball-relative`), false);
    t.is(relativeUrl.href, `http://localhost:${address.port}/download/tar.gz`);
});
test('calculate-download-checksum stream', async (t) => {
    ...
    const apiClient = api('ATOKEN');
    const shasum = await stream(apiClient, `http://localhost:${address.port}/redirect`, 'sha256');
    t.is(shasum, 'b94d27b9934d3e08a52e52d7da7dabfac484efe37a5380ee9088f7ace2efcde9' // sha256 of 'hello world'
    );
    t.teardown(() => new Promise((resolve) => {
        server.close(() => resolve());
    }));
});
//# sourceMappingURL=calculate-download-checksum-test.js.map