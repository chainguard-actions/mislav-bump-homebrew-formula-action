...
export function commitForRelease(messageTemplate, params = {}) {
...
export async function prepareEdit(ctx, sameRepoClient, crossRepoClient) {
    ...
    else if (owner.toLowerCase() == context.repo.owner.toLowerCase() &&
    ...
    const filePath = getInput('formula-path') || formulaPath(owner, repo, formulaName);
    const version = tagName.replace(/^v(\d)/, '$1');
    const downloadUrl = getInput('download-url') ||
    ...
    if (makePR === true && makeBranch === false) {
        throw new Error('create-pullrequest cannot be true if create-branch is false');
    }
    ...
    if (downloadUrl.endsWith('.git')) {
        ...
        replacements.set('sha256', getInput('download-sha256') ||
